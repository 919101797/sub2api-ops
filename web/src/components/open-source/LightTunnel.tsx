// Upstream: https://reactbits.dev/backgrounds/light-tunnel (React Bits, MIT + Commons Clause)
...
const LightTunnel: React.FC<LightTunnelProps> = ({
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      ...
      const requestedDpr = dpr ?? Math.min(window.devicePixelRatio || 1, 2);
      const budgetDpr = maxPixelCount
        ? Math.sqrt(maxPixelCount / Math.max(w * h, 1))
        : requestedDpr;
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      targetMouse = [
        ...
        Math.min(Math.max(1.0 - (e.clientY - rect.top) / rect.height, 0), 1)
      ];
    };
    const handleMouseLeave = () => {
      targetMouse = [0.5, 0.5];
    };
    if (!interaction) window.addEventListener('mousemove', handleMouseMove, { passive: true });
    ...
    const paintInteraction = (t: number) => {
      ...
      program.uniforms.iTime.value = frame.progress * 9;
      program.uniforms.uSize.value = size * (1 - frame.progress * 0.28);
      program.uniforms.uBrightness.value = brightness * (0.76 + frame.progress * 0.48);
      program.uniforms.uMouseOffset.value[0] = (frame.x - 0.5) * mouseStrength;
      program.uniforms.uMouseOffset.value[1] = (0.5 - frame.y) * mouseStrength;
    };
    ...
    const loop = (t: number) => {
      ...
      lastPaint = t;
      if (!interaction) (program.uniforms.iTime as { value: number }).value = (t - t0) * 0.001;
      ...
      paintInteraction(t);
      renderer.render({ scene: mesh });
      if (!interaction?.settled()) raf = requestAnimationFrame(loop);
    };

    const tryStart = () => {
      if (!paused && isVisible && isPageVisible && raf === 0 && !interaction?.settled()) {
    ...
    if (interaction) interaction.wake = tryStart;
  ...
  }, [dpr, maxPixelCount, paused, interaction, brightness, size, mouseStrength]);
  ...
  useEffect(() => {
    ...
    if (interaction) {
      const frame = interaction.sample(performance.now());
      u.iTime.value = frame.progress * 9;
      u.uSize.value = size * (1 - frame.progress * 0.28);
      u.uBrightness.value = brightness * (0.76 + frame.progress * 0.48);
    }
